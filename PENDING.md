...
## Lizimas <-> Jumia product linking (September 2026)

Why: Ryan wants to (in future) link products from Lizimas directly to Jumia, and let vendors who already sell on Jumia pull their existing Jumia listings into Lizimas. The mechanism is Jumia's own Vendor Center: a vendor creates an "Application" under their Jumia Settings > Applications (the same screen shown in Ryan's reference screenshots, with a Client ID and Client Secret), pastes those two values into Lizimas, and this app calls Jumia's Vendor API on their behalf from then on. That Applications row, previously a "Coming soon" stub in the vendor mobile app's Settings screen, is now the entry point into this feature.

What shipped: a new `vendor_jumia_connections` table (encrypted Client Secret + access/refresh tokens, reusing the AES-256-GCM helper added for vendor KYC - `server/utils/encryption.js` - despite its KYC-scoped env var name), `jumia_product_links` (one row per Lizimas<->Jumia product pairing with a sync status), and `jumia_sync_log` (an audit trail), all in `migrations/082_vendor_jumia_integration.sql`. `server/services/jumiaClient.js` is the actual Jumia HTTP client (OAuth2 client_credentials token exchange/refresh, product create/update, product list) and `server/services/jumiaSyncService.js` is the orchestration layer (connect/disconnect/test, push one or many products, list a vendor's Jumia products for import, import selected ones as new Lizimas products). `server/controllers/jumiaController.js` and nine new routes under `/api/vendors/me/jumia/...` expose all of that. On the vendor mobile app (`client/vendor/dashboard.html` + `client/js/vendor-mobile.js`), Settings > Applications is now a real screen: paste Client ID/Secret to connect, see connection status and shop name, disconnect, test the connection, see a status list of every linked product (Synced/Pending/Failed/Changed since last sync), and a "Push to Jumia" bulk action next to Activate/Deactivate/Delete on the Manage Products screen, plus an Import from Jumia screen that lists a vendor's existing Jumia products (already-imported ones shown disabled) for one-tap import.

Important caveat - please read before connecting a real vendor account: `server/services/jumiaClient.js`'s endpoint paths and field names (the OAuth token endpoint, the product create/update/list shapes) are this feature's one unverified piece. Jumia's Vendor Center API docs (vendorcenter.jumia.com/api-docs/) require a logged-in Jumia session to view, and every Jumia API host is unreachable from this dev environment's network entirely (confirmed via direct test - the outbound proxy here blocks it as against org policy), so these were written against the standard OAuth2 + REST/JSON shape Jumia's own documentation titles describe, not tested against a real Application. Everything else - the database schema, encryption, the vendor UI, sync status tracking, bulk push, import - was built and verified (tag-balance/duplicate-id/id-cross-reference checks, the full existing test suite at 204/204, and an interactive Playwright click-through of connect/push-status/import/disconnect against a mocked API that all passed cleanly with zero console errors from the new code). The one remaining step is a real test: create an Application in your own Jumia Vendor Center, paste its Client ID/Secret in, and see what actually comes back - `jumiaClient.js`'s constants and its two field-mapping functions are the only things that should need adjusting from that, everything else is independent of the exact Jumia field names.

Not covered by automated tests: any live call to Jumia's real API (network access alone rules this out from here); the exact category/attribute mapping Jumia may require per product category (currently passed through as free-form attributes from Lizimas' product_specifications); token refresh against a real expiring Jumia token.

## Jumia integration: desktop Applications panel, and real diagnostics on the first live connection attempt (September 2026)

Why: the Jumia connect/push/import UI above only ever existed in the vendor mobile shell (client/vendor/dashboard.html's .vendor-mobile-shell), since that's where Ryan's original screenshots were from. Testing from a desktop browser meant the wide-sidebar .vendor-desktop-shell's Account tab had nothing - just Profile Photo/Notices/Log Out. Separately, Ryan's first real connection attempt against his own Jumia Vendor Center account (Client ID 9e9c54e7-d813-4c58-ad2a-b2c5496e21b7) failed with only the generic "Could not connect to Jumia with the Client ID/Secret provided." - exactly the risk flagged in the entry above, but with no detail on screen or in logs to act on.

What shipped: a new "Applications" panel (plus a "Product Sync" and an "Import from Jumia" panel) added to the desktop shell's Account tab in client/vendor/dashboard.html, styled to match the rest of the desktop panel/table conventions and using the same padlock (rotate secret)/checkmark (test)/trash (disconnect) icon pattern as the mobile screen. A parallel set of vd*Jumia* functions was added to client/js/vendor-dashboard.js (vdLoadJumia, vdConnectJumia, vdDisconnectJumia, vdTestJumiaConnection, vdShowUpdateJumiaSecret/vdCancelUpdateJumiaSecret/vdSaveUpdatedJumiaSecret, vdLoadJumiaLinks/vdRenderJumiaLinks, vdShowJumiaImport/vdHideJumiaImport/vdLoadJumiaImport/vdRenderJumiaImportList/vdToggleJumiaImportSelect/vdImportSelectedJumiaProducts), mirroring the existing vm*Jumia* functions' logic but targeting vd-jumia-* element ids; both sets share the same vmJumiaConnectionCache/vmJumiaRemoteProductsCache/vmJumiaImportSelected state and backend endpoints, since the connection is a single source of truth either way. The Account tab's click handler in setupVendorTabs() now also calls vdLoadJumia(). The desktop products bulk-action bar gained a "Push to Jumia" button that calls the existing vmPushSelectedToJumia() directly (it only touches vendorProductsSelected/vmJumiaConnectionCache/alerts, no vm-prefixed DOM, so it's safe to call from either shell - the same pattern the pre-existing Activate/Deactivate/Delete buttons already use for bulkVendorProductAction()).

Caught while building this: the new #vd-jumia-update-secret-view div paired the `hidden` attribute with an inline `display:flex`, the exact same conflict class fixed in the September 2026 bulk-bar/overlay commit above (an inline style always beats the browser's default `[hidden]{display:none}` rule). Fixed the same way - removed the inline `display:flex` and added a scoped `.vendor-desktop-shell #vd-jumia-update-secret-view:not([hidden])` rule to client/css/vendor-desktop.css. Caught by an interactive Playwright click-through (padlock -> the sub-form stayed visibly rendered after Save, which static checks can't detect), not static checks alone.

Separately, improved server/services/jumiaClient.js's normalizeAxiosError so the FIRST live connection attempt's failure - the most likely outcome, since the endpoint paths are still unverified - is actually diagnosable: when Jumia's response doesn't carry a recognized error_description/message/error field, the HTTP status and a snippet of the raw response body (or the request's error code, e.g. ENOTFOUND/ECONNREFUSED/timeout) are now appended to the message shown in the vendor UI, and the full detail is also logged server-side via console.error. Previously a request that failed in an unrecognized shape (wrong path -> HTML 404, wrong host, timeout) fell back to a bare generic message with no way to tell those apart. This does not fix the underlying unverified-endpoint risk - it makes the next failure (or success) show exactly what Jumia's server returned, which is what's needed to correct jumiaClient.js's constants/field-mapping if the paths are wrong.

Verified: `node --check` clean on every touched JS file; HTML tag-balance, duplicate-id, and full getElementById/onclick cross-reference checks all clean (the `<path>`/`<polyline>` SVG tags falsely flagged by a naive `<p...>` regex are not `<p>` elements); the full existing test suite at 204/204; and an interactive Playwright click-through of the desktop panel at 1440px (connect, shop name/Client ID display, padlock -> rotate secret -> save, test connection, Import from Jumia -> select -> import, disconnect, and the Manage Products bulk bar's Push to Jumia button) with zero console errors from the new code.

Not covered by automated tests: the same live-Jumia-API gap as above - what Ryan's next real connection attempt actually returns, now that the error message will show it.

## Jumia integration: corrected the credential model (Refresh Token, not Client Secret) and the API host (September 2026)

Why: Ryan's first live connection attempt failed with only a generic error (see the entry above about improved diagnostics). That improved error message showed the real problem: Jumia's server responded HTTP 301 with the body of its own Vendor Center *frontend* app (`<title>Jumia | Vendor Center</title>`) - proof the request was landing on their web app, not an API host, because `JUMIA_API_BASE` was `vendorcenter.jumia.com/api`.

Researched a third-party open-source Jumia Vendor Center integration (github.com/damurka/jumia-vendor-mcp) whose README documents a working setup against the real API: base host `vendor-api.jumia.com`, catalog endpoints under `/catalog/products` (feed-based - create and update both POST/PATCH the same path, no id in the URL), and - critically - that a Jumia "Self Authorization" Application's "Generate Token" action issues a **Refresh Token**, not a Client Secret for a `client_credentials` grant. Asked Ryan to check his own real Application screen before changing anything this size: he confirmed he did see "Refresh Token" / "Generate Token" there, separate from what he'd been calling the Client Secret.

What changed: `JUMIA_API_BASE` now defaults to `https://vendor-api.jumia.com`; `JUMIA_PRODUCTS_PATH` to `/catalog/products`. `jumiaClient.js`'s `exchangeCredentialsForToken`/`refreshAccessToken` pair (client_credentials grant) was collapsed into one `mintAccessToken(clientId, refreshToken)` function using `grant_type: refresh_token` - there's no separate "exchange" step, since the vendor's pasted Refresh Token is itself the durable credential re-used on every access-token mint (uncertain whether Jumia rotates it on use; if it does, `data.refresh_token` from the response is available to the caller but nothing currently persists a rotated value over the original - punted on until real behaviour is seen). `jumiaSyncService.js` and `jumiaController.js` were updated to match; the `client_secret_enc` column and `clientSecret` parameter names were deliberately kept as-is (no migration) since nothing has ever connected successfully yet and every layer treats it as an opaque encrypted credential - a comment documents why. The wire field the frontend sends is now `refresh_token` (the controller still accepts the old `client_secret` key too, for safety). Every "Client Secret" label, placeholder, help string and button ("Update Secret" -> "Update Refresh Token", "Save New Secret" -> "Save New Refresh Token") was relabeled to "Refresh Token" across both the mobile and desktop Applications panels - the underlying element ids were left unchanged to minimize churn.

Verified: `node --check` clean on every touched file; the full existing test suite at 204/204; and a fresh interactive Playwright click-through of the desktop panel confirming the connect and secret-rotation requests now carry a `refresh_token` key (not `client_secret`) end to end, with the rest of the flow (shop name, links list, test connection, import, disconnect, Push to Jumia) unaffected.

Not covered by automated tests: still the same live-Jumia-API gap - whether `vendor-api.jumia.com`, the token path, and `/catalog/products` are exactly right can only be confirmed by Ryan's next real attempt, which will now show the actual HTTP status/body if it's still wrong (per the diagnostics entry above).

## Jumia integration: correct token endpoint, confirmed from Jumia's own official docs (September 2026)

Why: even after the host fix and the Refresh-Token credential correction above, Ryan's next attempt returned "no Route matched with those values" - a standard Kong API Gateway 404, meaning the request now reached Jumia's real API (vendor-api.jumia.com) but hit a path Kong has no route for (`/oauth/token`, still a guess at that point).

Ryan found and opened Jumia's own official public Postman workspace (postman.com/jumiagandalf/jumia-vendor-api, "Vendor API Collection", author Pedro Ferreira) and sent screenshots/the full documentation text for its "Obtain an Access Token from Authorization Code / Refresh Token" request - the authoritative source this integration has been missing. It confirms: the endpoint is `{{baseUrl}}/token` (not `/oauth/token`), the example cURL uses `vendor-api-staging.jumia.com` (staging - production is presumably `vendor-api.jumia.com`, already in use), and - critically - the request body is `application/x-www-form-urlencoded`, NOT JSON. For "Case 2 (Refresh Token)" specifically: `client_id`, `grant_type=refresh_token`, and `refresh_token` are required; `client_secret`/`code`/`redirect_uri` are Case-1-only (the Authorization Code flow this app doesn't use). Response: `{ access_token, expires_in, refresh_token, refresh_expires_in, token_type }`.
...
## Jumia: multiple named Applications per vendor, plus an explicit Export-to-Jumia panel (September 2026)

Why: two follow-on requests from Ryan. First, the Product Sync panel had an Import button but nothing for pushing Lizimas products to Jumia (there was already a bulk "Push to Jumia" action buried in the products table's bulk-action bar, but nothing under Applications/Sync itself). Second, and bigger: Jumia's real Vendor Center lets a seller register several Applications (Self Authorization or Web Application/OAuth), and Ryan wants the same here - the old build supported exactly one connection per vendor.

What shipped, schema: `migrations/084_vendor_jumia_multi_application.sql` (written, **not yet run** - Ryan runs migrations himself) drops the one-connection-per-vendor unique constraint on `vendor_jumia_connections` and adds `name`, `app_type` (`self_authorization`|`web_application`), `redirect_uri`, and `is_active` (partial-unique index - exactly one active row per vendor, that's the one used for sync). A vendor's first successful connection auto-activates, so existing single-connection behavior needs zero extra steps from a vendor's point of view.

Backend: `jumiaSyncService.js`'s connection lifecycle was rewritten around Applications (`listApplications`, `createApplication`, `deleteApplication`, `setActiveApplication`, `connectApplication`, `disconnectApplication`, `setWebApplicationCredentials`, `getAuthorizeUrl`, `handleOAuthCallback`, `testApplicationConnection`). Every push/pull/import function was left completely untouched - they all resolve "the active application" transparently inside `getFreshConnection`, so none of that logic had to change or be re-tested from scratch. `jumiaController.js` and `server/routes/vendors.js` gained the matching Applications CRUD routes plus a public (unauthenticated, state-signed via JWT) OAuth callback route for the new Web Application flow.

Frontend: both the desktop Account tab (`#vd-jumia-*`) and the mobile Settings > Applications screen (`#vm-jumia-*`, `vendor-mobile.js`) were rebuilt around an Applications table/list with "+ Create Application" (name + the same two type options Jumia's own dialog shows), a per-application setup panel, Make Active / Test / Delete actions, and a new Export-to-Jumia panel (product picker + bulk push, next to the existing Import-from-Jumia panel).

**Important - flagging this clearly since Ryan asked to push to live after this is verified**: Self Authorization (refresh_token grant) is the flow already confirmed working against Ryan's real Jumia account from an earlier session - untouched here beyond being one of two Application types instead of the only one. The **Web Application (OAuth authorization_code) flow is new and unverified against a real Jumia account** - the token exchange reuses the confirmed `/token` endpoint, but the browser-facing `/authorize` consent screen URL (`JUMIA_AUTHORIZE_BASE`/`JUMIA_AUTHORIZE_PATH` env vars, default `https://vendorcenter.jumia.com/oauth/authorize`) is a best-effort guess from Jumia's Postman docs, not something anyone has clicked through end to end yet. This is flagged in the code, and in the vendor-facing UI itself (a yellow warning box on that setup panel) - so a vendor trying it sees the caveat before they rely on it. Self Authorization is what to recommend Ryan test first.

Verified: `node --check` clean on every touched file; full test suite 204/204 (push/pull/import logic untouched); no duplicate ids or unbalanced tags. Found and fixed a real rendering bug while testing: `#vd-jumia-create-application-view` and `#vd-jumia-app-setup-view` both carried an inline `style="...display:flex..."` alongside the `hidden` attribute - since an inline style always outranks the browser's default `[hidden]{display:none}` rule, both panels would have stayed visibly open at all times regardless of what the JS set `.hidden` to. Fixed by having the show/hide functions also set `style.display` directly, and by changing each element's initial inline style to `display:none` so they start correctly closed before any JS runs. Playwright-tested end to end afterward: creating a Self Authorization application, connecting it with mock credentials, seeing it render "Connected" + "ACTIVE" in the table, Test Connection, creating a second (Web Application-type) application and confirming its redirect-URI field and unverified-flow warning show correctly, Export to Jumia (product list loads, selecting products updates the count, Push Selected calls the bulk-push endpoint with the right product ids), and Delete - all on both the desktop table and the mobile Applications list (including the 2-step mobile create flow and the Settings screen's "Connected"/"N Applications" status line, which needed a small fix of its own since nothing had been refreshing it outside of actually visiting the Applications screen).

Not yet done: the migration hasn't been run (that's Ryan's step), and the Web Application OAuth flow's `/authorize` URL needs a real click-through against Jumia to confirm or correct.
...
## Admin panel: Jumia Applications for Lizimas's own products (September 2026)

Why: Ryan asked for the same Jumia Applications feature vendors have, but inside the admin panel, so he can push Lizimas's own catalogue (not vendor products) to Jumia through the store's own Jumia Vendor Center account. Confirmed scope with him directly: this only ever touches products with `vendor_id IS NULL` - a vendor's own products stay on the vendor's own Jumia path, untouched by any of this.

Rather than generalize the existing vendor-scoped Jumia system to support a nullable "store-level" owner, this ships as a **fully parallel, independent system** - separate tables, service, controller and routes - specifically so nothing about the vendor Jumia feature Ryan is already relying on in production could regress. The low-level `jumiaClient.js` HTTP helpers are the only thing shared, since they already take a raw DB connection row rather than a vendor-specific shape.

What shipped, schema: `migrations/085_admin_jumia_integration.sql` (written, **not yet run** - Ryan runs migrations himself) adds `admin_jumia_connections` (mirrors `vendor_jumia_connections` minus any vendor id - a unique index on `is_active` enforces one active row across the whole table, since there's only one store), `admin_jumia_product_links`, and `admin_jumia_sync_log` (both mirror their vendor equivalents minus `vendor_id`).

Backend: `server/services/jumiaAdminSyncService.js` is a full parallel of `jumiaSyncService.js` with every `vendorId` parameter removed - same Applications lifecycle (create/connect/disconnect/test/activate/delete, both Self Authorization and Web Application/OAuth types), same push/pull/import functions, scoped everywhere by `WHERE vendor_id IS NULL` when loading a product to push. The OAuth `state` JWT is signed with `{scope: "admin", applicationId}` instead of `{vendorId, applicationId}`, so the callback handler can tell an admin sign-in apart from a vendor one using the same `JWT_SECRET`. `server/controllers/jumiaAdminController.js` is the matching HTTP layer, and `server/routes/admin.js` mounts it under `/api/admin/jumia/*` (the OAuth callback route is public, ahead of the `requireAuth+requireAdmin` gate; everything else sits behind it).

Frontend: the admin Products tab got a new "Jumia" toggle button next to Import/Export, opening an Applications panel that's a direct mirror of the vendor desktop Applications table (create, connect/reconnect, test, make active, delete) plus Export-to-Jumia and Import-from-Jumia panels. The Export list reuses the product table admin.js already loads (`adminProducts`) filtered to `!p.vendor_id`, rather than adding a new listing endpoint. A small OAuth-return handler switches to the Products tab and opens the panel automatically when Jumia's redirect lands back on `/admin.html?jumia_oauth=...#products`.

Verified: `node --check` clean on every touched/new file; the full route tree (`server/routes/admin.js` and the whole `server/app.js`) loads without error, confirming `jumiaAdminController.js` actually resolves (a controller file that was written but not yet copied into place was caught exactly this way before verification, and fixed); full test suite 204/204; no duplicate ids or unbalanced tags in `admin.html`. Playwright-tested end to end against mocked APIs: the panel toggles open, both Applications render with the right type/status/active pill, creating a new Application opens its setup view automatically, the Self Authorization and Web Application setup views show the right fields (including a populated redirect URI), Export to Jumia's product list correctly includes Lizimas's own product and excludes a vendor's product, Push Selected calls the bulk-push endpoint with the right id, Import from Jumia lists remote items and marks already-linked ones, and the OAuth-return handler cleans the URL after showing its result.

Not yet done: the migration hasn't been run (Ryan's step), and - same caveat as the vendor Jumia feature - the Web Application OAuth flow has not been click-tested against a real Jumia account.
