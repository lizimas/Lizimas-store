...
- Session tokens now live in per-portal httpOnly cookies (lz_s_user / lz_s_vendor /
  lz_s_admin / lz_s_staff), chosen by the X-LZ-Session header that
  client/js/lz-session.js adds to every /api request. Cookie logins need that
  header (CSRF guard) and the cookies are SameSite=Lax. localStorage keeps only
  the placeholder "cookie". Old tokens are adopted into the cookie on first
  load; removing a token key signs the server session out (POST /api/auth/logout).
  See server/utils/sessionCookie.js.
- googleCallback/facebookCallback no longer fake a request/response: sign-in
  functions take opts.respond, and completeLogin reports through it.
- The unused server/middleware/auth.js was removed.
...
- **No real per-category commission rates yet.** The PDF's Channel-benchmark
...
Adds the "Seller Information" box Ryan asked for after sharing
screenshots of a Channel product page: a store link, a seller score
...
- **Why boost-to-top rather than a separate carousel**: a dedicated
  ...
  existing listings mirrors real placement-boost systems (Amazon/Channel
...
Ryan supplied a Channel Uganda 2025-benchmarked rate card (20 categories) and asked for it to be "editable from the admin panel." The admin Categories tab already had a per-category "Rate" editor wired to `commission_rules` since Task #52/53 - nothing new to build there. The actual work was mapping Ryan's 20 buckets onto Lizimas' real category tree, which doesn't look anything flat ~20-category structure (fetched live from `GET /api/products/categories` in production - 230 categories, 6 top-level, 3 levels deep).
...
1. An **instructions modal** on page load (the channel's screenshot 3) - explains a valid email/phone is required, that the email can't be changed after registration, and that every field on every step is mandatory. "Got it, let's start" dismisses it.
...
Wired a mobile app shell into the vendor dashboard, replacing the desktop sidebar-and-tabs layout below a ~768px breakpoint with a persistent Home / Orders / Manage Products / Menu bottom nav, matching real screenshots of the channel's seller-center app. Built directly into `client/vendor/dashboard.html` (Ryan's explicit call) rather than as a separate page - same `vendorToken` session, same API calls, existing desktop tabs untouched for wider screens.
...
- The mockup's mobile Orders page used the channel's own status vocabulary (Pending/Ready to Ship/Shipped/Delivered/Cancelled/Delivery Failed/Returned). The real wiring uses Lizimas' actual stages instead (`server/utils/vendorOrderStage.js`: New/Accepted/Processing/Ready for Handover/Handed Over/In Delivery/Delivered/Rejected at Inspection/Return in Progress/Forfeited/Cancelled) - richer and more accurate than what the mockup guessed at.
...
## Vendor panel reorganized to match Vendor Center; bulk-action confirmation + results report; specifications paste-from-Excel (September 2026)

Requested after the vendor sent real screenshots of the live Channel Vendor
Center (the channel's vendor site) alongside a long architectural proposal for a
...
reorganizing the sidebar to mirror the real Channel navigation, leaving the
...
- **Sidebar reorganized to mirror the real Vendor Center screenshots**:
  ...
  Channel features Lizimas has no equivalent for - multi-shop switching,
  Fulfillment by Lizimas, Sponsored Products/Advertise - were deliberately not
  cloned as hollow placeholders. Orders was kept as an expandable group
  (unlike the channel's flat Orders item) so Returns/Refunds still have somewhere
  ...
  rather than adopting the channel's two-card wizard, per the vendor's explicit
  instruction.
...
## Lizimas <-> Channel product linking (September 2026)

Why: Ryan wants to (in future) link products from Lizimas directly to Channel, and let vendors who already sell on Channel pull their existing Channel listings into Lizimas. The mechanism is the channel's own Vendor Center: a vendor creates an "Application" under their Channel Settings > Applications (the same screen shown in Ryan's reference screenshots, with a Client ID and Client Secret), pastes those two values into Lizimas, and this app calls the channel's Vendor API on their behalf from then on. That Applications row, previously a "Coming soon" stub in the vendor mobile app's Settings screen, is now the entry point into this feature.

What shipped: a new `vendor_channel_connections` table (encrypted Client Secret + access/refresh tokens, reusing the AES-256-GCM helper added for vendor KYC - `server/utils/encryption.js` - despite its KYC-scoped env var name), `channel_product_links` (one row per Lizimas<->Channel product pairing with a sync status), and `channel_sync_log` (an audit trail), all in `migrations/082_vendor_channel_integration.sql`. `server/services/channelClient.js` is the actual Channel HTTP client (OAuth2 client_credentials token exchange/refresh, product create/update, product list) and `server/services/channelSyncService.js` is the orchestration layer (connect/disconnect/test, push one or many products, list a vendor's Channel products for import, import selected ones as new Lizimas products). `server/controllers/channelController.js` and nine new routes under `/api/vendors/me/channel/...` expose all of that. On the vendor mobile app (`client/vendor/dashboard.html` + `client/js/vendor-mobile.js`), Settings > Applications is now a real screen: paste Client ID/Secret to connect, see connection status and shop name, disconnect, test the connection, see a status list of every linked product (Synced/Pending/Failed/Changed since last sync), and a "Push to Channel" bulk action next to Activate/Deactivate/Delete on the Manage Products screen, plus an Import from Channel screen that lists a vendor's existing Channel products (already-imported ones shown disabled) for one-tap import.

Important caveat - please read before connecting a real vendor account: `server/services/channelClient.js`'s endpoint paths and field names (the OAuth token endpoint, the product create/update/list shapes) are this feature's one unverified piece. the channel's Vendor Center API docs (the channel's vendor site/api-docs/) require a logged-in Channel session to view, and every Channel API host is unreachable from this dev environment's network entirely (confirmed via direct test - the outbound proxy here blocks it as against org policy), so these were written against the standard OAuth2 + REST/JSON shape the channel's own documentation titles describe, not tested against a real Application. Everything else - the database schema, encryption, the vendor UI, sync status tracking, bulk push, import - was built and verified (tag-balance/duplicate-id/id-cross-reference checks, the full existing test suite at 204/204, and an interactive Playwright click-through of connect/push-status/import/disconnect against a mocked API that all passed cleanly with zero console errors from the new code). The one remaining step is a real test: create an Application in your own Vendor Center, paste its Client ID/Secret in, and see what actually comes back - `channelClient.js`'s constants and its two field-mapping functions are the only things that should need adjusting from that, everything else is independent of the exact Channel field names.

Not covered by automated tests: any live call to the channel's real API (network access alone rules this out from here); the exact category/attribute mapping Channel may require per product category (currently passed through as free-form attributes from Lizimas' product_specifications); token refresh against a real expiring Channel token.

## Channel integration: desktop Applications panel, and real diagnostics on the first live connection attempt (September 2026)

Why: the Channel connect/push/import UI above only ever existed in the vendor mobile shell (client/vendor/dashboard.html's .vendor-mobile-shell), since that's where Ryan's original screenshots were from. Testing from a desktop browser meant the wide-sidebar .vendor-desktop-shell's Account tab had nothing - just Profile Photo/Notices/Log Out. Separately, Ryan's first real connection attempt against his own Vendor Center account (Client ID 9e9c54e7-d813-4c58-ad2a-b2c5496e21b7) failed with only the generic "Could not connect to Channel with the Client ID/Secret provided." - exactly the risk flagged in the entry above, but with no detail on screen or in logs to act on.

What shipped: a new "Applications" panel (plus a "Product Sync" and an "Import from Channel" panel) added to the desktop shell's Account tab in client/vendor/dashboard.html, styled to match the rest of the desktop panel/table conventions and using the same padlock (rotate secret)/checkmark (test)/trash (disconnect) icon pattern as the mobile screen. A parallel set of vd*Channel* functions was added to client/js/vendor-dashboard.js (vdLoadChannel, vdConnectChannel, vdDisconnectChannel, vdTestChannelConnection, vdShowUpdateChannelSecret/vdCancelUpdateChannelSecret/vdSaveUpdatedChannelSecret, vdLoadChannelLinks/vdRenderChannelLinks, vdShowChannelImport/vdHideChannelImport/vdLoadChannelImport/vdRenderChannelImportList/vdToggleChannelImportSelect/vdImportSelectedChannelProducts), mirroring the existing vm*Channel* functions' logic but targeting vd-channel-* element ids; both sets share the same vmChannelConnectionCache/vmChannelRemoteProductsCache/vmChannelImportSelected state and backend endpoints, since the connection is a single source of truth either way. The Account tab's click handler in setupVendorTabs() now also calls vdLoadChannel(). The desktop products bulk-action bar gained a "Push to Channel" button that calls the existing vmPushSelectedToChannel() directly (it only touches vendorProductsSelected/vmChannelConnectionCache/alerts, no vm-prefixed DOM, so it's safe to call from either shell - the same pattern the pre-existing Activate/Deactivate/Delete buttons already use for bulkVendorProductAction()).

Caught while building this: the new #vd-channel-update-secret-view div paired the `hidden` attribute with an inline `display:flex`, the exact same conflict class fixed in the September 2026 bulk-bar/overlay commit above (an inline style always beats the browser's default `[hidden]{display:none}` rule). Fixed the same way - removed the inline `display:flex` and added a scoped `.vendor-desktop-shell #vd-channel-update-secret-view:not([hidden])` rule to client/css/vendor-desktop.css. Caught by an interactive Playwright click-through (padlock -> the sub-form stayed visibly rendered after Save, which static checks can't detect), not static checks alone.

Separately, improved server/services/channelClient.js's normalizeAxiosError so the FIRST live connection attempt's failure - the most likely outcome, since the endpoint paths are still unverified - is actually diagnosable: when the channel's response doesn't carry a recognized error_description/message/error field, the HTTP status and a snippet of the raw response body (or the request's error code, e.g. ENOTFOUND/ECONNREFUSED/timeout) are now appended to the message shown in the vendor UI, and the full detail is also logged server-side via console.error. Previously a request that failed in an unrecognized shape (wrong path -> HTML 404, wrong host, timeout) fell back to a bare generic message with no way to tell those apart. This does not fix the underlying unverified-endpoint risk - it makes the next failure (or success) show exactly what the channel's server returned, which is what's needed to correct channelClient.js's constants/field-mapping if the paths are wrong.

Verified: `node --check` clean on every touched JS file; HTML tag-balance, duplicate-id, and full getElementById/onclick cross-reference checks all clean (the `<path>`/`<polyline>` SVG tags falsely flagged by a naive `<p...>` regex are not `<p>` elements); the full existing test suite at 204/204; and an interactive Playwright click-through of the desktop panel at 1440px (connect, shop name/Client ID display, padlock -> rotate secret -> save, test connection, Import from Channel -> select -> import, disconnect, and the Manage Products bulk bar's Push to Channel button) with zero console errors from the new code.

Not covered by automated tests: the same live-Channel-API gap as above - what Ryan's next real connection attempt actually returns, now that the error message will show it.

## Channel integration: corrected the credential model (Refresh Token, not Client Secret) and the API host (September 2026)

Why: Ryan's first live connection attempt failed with only a generic error (see the entry above about improved diagnostics). That improved error message showed the real problem: the channel's server responded HTTP 301 with the body of its own Vendor Center *frontend* app (`<title>Channel | Vendor Center</title>`) - proof the request was landing on their web app, not an API host, because `CHANNEL_API_BASE` was `the channel's vendor site/api`.

Researched a third-party open-source Vendor Center integration (github.com/damurka/channel-vendor-mcp) whose README documents a working setup against the real API: base host `vendor-api.channel.com`, catalog endpoints under `/catalog/products` (feed-based - create and update both POST/PATCH the same path, no id in the URL), and - critically - that a Channel "Self Authorization" Application's "Generate Token" action issues a **Refresh Token**, not a Client Secret for a `client_credentials` grant. Asked Ryan to check his own real Application screen before changing anything this size: he confirmed he did see "Refresh Token" / "Generate Token" there, separate from what he'd been calling the Client Secret.

What changed: `CHANNEL_API_BASE` now defaults to `(channel address)`; `CHANNEL_PRODUCTS_PATH` to `/catalog/products`. `channelClient.js`'s `exchangeCredentialsForToken`/`refreshAccessToken` pair (client_credentials grant) was collapsed into one `mintAccessToken(clientId, refreshToken)` function using `grant_type: refresh_token` - there's no separate "exchange" step, since the vendor's pasted Refresh Token is itself the durable credential re-used on every access-token mint (uncertain whether Channel rotates it on use; if it does, `data.refresh_token` from the response is available to the caller but nothing currently persists a rotated value over the original - punted on until real behaviour is seen). `channelSyncService.js` and `channelController.js` were updated to match; the `client_secret_enc` column and `clientSecret` parameter names were deliberately kept as-is (no migration) since nothing has ever connected successfully yet and every layer treats it as an opaque encrypted credential - a comment documents why. The wire field the frontend sends is now `refresh_token` (the controller still accepts the old `client_secret` key too, for safety). Every "Client Secret" label, placeholder, help string and button ("Update Secret" -> "Update Refresh Token", "Save New Secret" -> "Save New Refresh Token") was relabeled to "Refresh Token" across both the mobile and desktop Applications panels - the underlying element ids were left unchanged to minimize churn.

Verified: `node --check` clean on every touched file; the full existing test suite at 204/204; and a fresh interactive Playwright click-through of the desktop panel confirming the connect and secret-rotation requests now carry a `refresh_token` key (not `client_secret`) end to end, with the rest of the flow (shop name, links list, test connection, import, disconnect, Push to Channel) unaffected.

Not covered by automated tests: still the same live-Channel-API gap - whether `vendor-api.channel.com`, the token path, and `/catalog/products` are exactly right can only be confirmed by Ryan's next real attempt, which will now show the actual HTTP status/body if it's still wrong (per the diagnostics entry above).

## Channel integration: correct token endpoint, confirmed from the channel's own official docs (September 2026)

Why: even after the host fix and the Refresh-Token credential correction above, Ryan's next attempt returned "no Route matched with those values" - a standard Kong API Gateway 404, meaning the request now reached the channel's real API (vendor-api.channel.com) but hit a path Kong has no route for (`/oauth/token`, still a guess at that point).

Ryan found and opened the channel's own official public Postman workspace (postman.com/channelgandalf/channel-vendor-api, "Vendor API Collection", author Pedro Ferreira) and sent screenshots/the full documentation text for its "Obtain an Access Token from Authorization Code / Refresh Token" request - the authoritative source this integration has been missing. It confirms: the endpoint is `{{baseUrl}}/token` (not `/oauth/token`), the example cURL uses `vendor-api-staging.channel.com` (staging - production is presumably `vendor-api.channel.com`, already in use), and - critically - the request body is `application/x-www-form-urlencoded`, NOT JSON. For "Case 2 (Refresh Token)" specifically: `client_id`, `grant_type=refresh_token`, and `refresh_token` are required; `client_secret`/`code`/`redirect_uri` are Case-1-only (the Authorization Code flow this app doesn't use). Response: `{ access_token, expires_in, refresh_token, refresh_expires_in, token_type }`.

What changed: `CHANNEL_TOKEN_PATH` corrected to `/token`. `mintAccessToken()` in `channelClient.js` now POSTs `application/x-www-form-urlencoded` (via `URLSearchParams`, a one-off `axios.post` call rather than the shared JSON `channelHttp()` client used elsewhere) instead of a JSON body - this was silently wrong before (the channel's Kong gateway would have rejected a JSON body at the right path too, not just the wrong path). The response field mapping was already correct and unchanged.
...
## Channel integration: first successful live connection (September 2026)

Ryan connected his real Vendor Center account (Client ID 9e9c54e7-d813-4c58-ad2a-b2c5496e21b7) successfully after the /token path + form-urlencoded body fix above - the vendor UI shows "Connected" with his shop name. This confirms the entire auth layer (host, path, credential model, request/response shapes) end to end against production, not just against a mock.

Still unverified: everything past auth - pushing a Lizimas product to Channel and importing a Channel product into Lizimas, which hit /catalog/products (a guess, not confirmed like the token endpoint was). That's the next real test.
...
## Vendor product upload brought to parity with staff; specs paste added to staff; description-block ownership gap closed (September 2026)

Why: Ryan asked for the vendor Add Product form to match what staff can do, and for a "can't select the boxes and paste there" complaint on Specifications. Investigation found no reproducible paste bug anywhere - a dedicated Playwright test successfully clicked, focused, and typed into both the label input and vendor's existing Paste-from-Excel box on the real markup. The actual gap was feature parity: staff's product form had no bulk-paste box at all (vendor already did), and vendor's form had no rich description-block editor (staff already did, unwired to any frontend).

What shipped: `client/js/description-block-editor.js` gained a `apiBase` option (default unchanged at `/api/products`) so the same editor can now mount against either staff's or vendor's endpoints. Vendor's Add/Edit Product form (`client/vendor/dashboard.html` + `vendor-dashboard.js`) now mounts it (`apiBase: "/api/vendors/products"`) and saves blocks alongside the rest of the product on submit, exactly mirroring how `admin.js` already did it. Staff's Specifications section (`admin.html` + `admin.js`) gained the same Paste-from-Excel box and parser vendor already had (`parseAndAddAdminSpecs`, ported directly from vendor's working version).

While wiring the vendor path to `saveDescriptionBlocks`, found it had no ownership check when reached via the vendor route (`requireAuth+requireVendor` only) - any vendor could have overwritten any other vendor's description blocks by guessing product ids. Fixed using the same `canEditProduct` pattern `updateProduct` already used, now exported from `productController.js`.

Verified: `node --check` clean on every touched file; full test suite 204/204; no duplicate ids or unbalanced tags in either `admin.html` or `vendor/dashboard.html`. Playwright-tested against the real markup: vendor's Add Product tab mounts the block editor, a Heading block can be added and typed into, and `LzBlockEditor.save()` round-trips the exact payload (`PUT /api/vendors/products/:id/description-blocks`) the backend route expects. Staff's paste box correctly parses tab-separated, double-space-separated, and colon-separated lines into spec rows, and clears itself on a fresh "+ Add Product".

## Channel: multiple named Applications per vendor, plus an explicit Export-to-Channel panel (September 2026)

Why: two follow-on requests from Ryan. First, the Product Sync panel had an Import button but nothing for pushing Lizimas products to Channel (there was already a bulk "Push to Channel" action buried in the products table's bulk-action bar, but nothing under Applications/Sync itself). Second, and bigger: the channel's real Vendor Center lets a seller register several Applications (Self Authorization or Web Application/OAuth), and Ryan wants the same here - the old build supported exactly one connection per vendor.

What shipped, schema: `migrations/084_vendor_channel_multi_application.sql` (written, **not yet run** - Ryan runs migrations himself) drops the one-connection-per-vendor unique constraint on `vendor_channel_connections` and adds `name`, `app_type` (`self_authorization`|`web_application`), `redirect_uri`, and `is_active` (partial-unique index - exactly one active row per vendor, that's the one used for sync). A vendor's first successful connection auto-activates, so existing single-connection behavior needs zero extra steps from a vendor's point of view.

Backend: `channelSyncService.js`'s connection lifecycle was rewritten around Applications (`listApplications`, `createApplication`, `deleteApplication`, `setActiveApplication`, `connectApplication`, `disconnectApplication`, `setWebApplicationCredentials`, `getAuthorizeUrl`, `handleOAuthCallback`, `testApplicationConnection`). Every push/pull/import function was left completely untouched - they all resolve "the active application" transparently inside `getFreshConnection`, so none of that logic had to change or be re-tested from scratch. `channelController.js` and `server/routes/vendors.js` gained the matching Applications CRUD routes plus a public (unauthenticated, state-signed via JWT) OAuth callback route for the new Web Application flow.

Frontend: both the desktop Account tab (`#vd-channel-*`) and the mobile Settings > Applications screen (`#vm-channel-*`, `vendor-mobile.js`) were rebuilt around an Applications table/list with "+ Create Application" (name + the same two type options the channel's own dialog shows), a per-application setup panel, Make Active / Test / Delete actions, and a new Export-to-Channel panel (product picker + bulk push, next to the existing Import-from-Channel panel).

**Important - flagging this clearly since Ryan asked to push to live after this is verified**: Self Authorization (refresh_token grant) is the flow already confirmed working against Ryan's real Channel account from an earlier session - untouched here beyond being one of two Application types instead of the only one. The **Web Application (OAuth authorization_code) flow is new and unverified against a real Channel account** - the token exchange reuses the confirmed `/token` endpoint, but the browser-facing `/authorize` consent screen URL (`CHANNEL_AUTHORIZE_BASE`/`CHANNEL_AUTHORIZE_PATH` env vars, default `https://the channel's vendor site/oauth/authorize`) is a best-effort guess from the channel's Postman docs, not something anyone has clicked through end to end yet. This is flagged in the code, and in the vendor-facing UI itself (a yellow warning box on that setup panel) - so a vendor trying it sees the caveat before they rely on it. Self Authorization is what to recommend Ryan test first.

Verified: `node --check` clean on every touched file; full test suite 204/204 (push/pull/import logic untouched); no duplicate ids or unbalanced tags. Found and fixed a real rendering bug while testing: `#vd-channel-create-application-view` and `#vd-channel-app-setup-view` both carried an inline `style="...display:flex..."` alongside the `hidden` attribute - since an inline style always outranks the browser's default `[hidden]{display:none}` rule, both panels would have stayed visibly open at all times regardless of what the JS set `.hidden` to. Fixed by having the show/hide functions also set `style.display` directly, and by changing each element's initial inline style to `display:none` so they start correctly closed before any JS runs. Playwright-tested end to end afterward: creating a Self Authorization application, connecting it with mock credentials, seeing it render "Connected" + "ACTIVE" in the table, Test Connection, creating a second (Web Application-type) application and confirming its redirect-URI field and unverified-flow warning show correctly, Export to Channel (product list loads, selecting products updates the count, Push Selected calls the bulk-push endpoint with the right product ids), and Delete - all on both the desktop table and the mobile Applications list (including the 2-step mobile create flow and the Settings screen's "Connected"/"N Applications" status line, which needed a small fix of its own since nothing had been refreshing it outside of actually visiting the Applications screen).

Not yet done: the migration hasn't been run (that's Ryan's step), and the Web Application OAuth flow's `/authorize` URL needs a real click-through against Channel to confirm or correct.

## Admin panel: product import/export (CSV/XLSX) (September 2026)

Why: the last piece of Ryan's "run and fix everything" batch - a full bulk import/export system for the product catalogue, directly in the admin panel.

Turned out the import side already existed as working, tested-looking backend code (`adminController.js`'s `importProducts`, wired to `POST /api/admin/products/import`) from an earlier session - but only the bare minimum fields (name, description, price, stock, category), nothing in the frontend called it, and there was no export at all.

What shipped: `importProducts` was extended to cover the full admin-editable field set the product form itself has - sku, brand, gtin, mpn, material, color, sleeve, style, length, fit, pattern, care_instructions, occasion, warranty_months, package_size (validated against the same four tiers the delivery pricing engine knows about), status (pending/approved/rejected - this route is admin-only, so there's no "flip back to pending" rule to preserve the way there is on the staff-facing form), and image (an already-hosted URL passes through unchanged; left blank on an update, the existing image is untouched). Rows now also match by `sku` when no `id` is given, not just by `id` - so a previously exported file can be re-uploaded to bulk-edit without anyone having to look up database ids by hand. New rows created this way are now correctly stamped with `created_by` (the importing admin), which the original version left null. The whole batch still runs inside one transaction exactly as before: a genuine unexpected DB error rolls back everything, while ordinary per-row validation problems (missing name, bad price, unknown status value) are collected and the row is skipped without aborting the rest.

`exportProducts` is new (`GET /api/admin/products/export`, optional `?status=` filter) - streams every non-deleted product as a CSV using exactly the columns `importProducts` understands, so export-edit-reimport is a real round trip. The admin Products tab got a new "Import / Export" panel next to "+ Add Product": a status dropdown + Download CSV button, and a file picker + Upload & Import button that shows a created/updated/skipped summary plus a per-row error list when something didn't parse.

Verified: `node --check` clean on every touched file; full route tree (`server/app.js`) loads without error; full test suite 204/204; no duplicate ids or unbalanced tags in `admin.html`. Playwright-tested end to end: the panel opens, Download CSV triggers a real file download with the expected filename and header row, the status filter is reflected in the request query string, uploading a CSV shows the created/updated/skipped counts and the row-level error detail, and the file input clears itself and the product list reloads after a successful import.

Not yet done: no real database has been hit (only the row-shaping logic and the transaction structure), so worth Ryan running one small real import first to confirm the category auto-create and sku-match-on-reimport behave as expected against production data before relying on it for anything large.

## Admin panel: Channel Applications for Lizimas's own products (September 2026)

Why: Ryan asked for the same Channel Applications feature vendors have, but inside the admin panel, so he can push Lizimas's own catalogue (not vendor products) to Channel through the store's own Vendor Center account. Confirmed scope with him directly: this only ever touches products with `vendor_id IS NULL` - a vendor's own products stay on the vendor's own Channel path, untouched by any of this.

Rather than generalize the existing vendor-scoped Channel system to support a nullable "store-level" owner, this ships as a **fully parallel, independent system** - separate tables, service, controller and routes - specifically so nothing about the vendor Channel feature Ryan is already relying on in production could regress. The low-level `channelClient.js` HTTP helpers are the only thing shared, since they already take a raw DB connection row rather than a vendor-specific shape.

What shipped, schema: `migrations/085_admin_channel_integration.sql` (written, **not yet run** - Ryan runs migrations himself) adds `admin_channel_connections` (mirrors `vendor_channel_connections` minus any vendor id - a unique index on `is_active` enforces one active row across the whole table, since there's only one store), `admin_channel_product_links`, and `admin_channel_sync_log` (both mirror their vendor equivalents minus `vendor_id`).

Backend: `server/services/channelAdminSyncService.js` is a full parallel of `channelSyncService.js` with every `vendorId` parameter removed - same Applications lifecycle (create/connect/disconnect/test/activate/delete, both Self Authorization and Web Application/OAuth types), same push/pull/import functions, scoped everywhere by `WHERE vendor_id IS NULL` when loading a product to push. The OAuth `state` JWT is signed with `{scope: "admin", applicationId}` instead of `{vendorId, applicationId}`, so the callback handler can tell an admin sign-in apart from a vendor one using the same `JWT_SECRET`. `server/controllers/channelAdminController.js` is the matching HTTP layer, and `server/routes/admin.js` mounts it under `/api/admin/channel/*` (the OAuth callback route is public, ahead of the `requireAuth+requireAdmin` gate; everything else sits behind it).

Frontend: the admin Products tab got a new "Channel" toggle button next to Import/Export, opening an Applications panel that's a direct mirror of the vendor desktop Applications table (create, connect/reconnect, test, make active, delete) plus Export-to-Channel and Import-from-Channel panels. The Export list reuses the product table admin.js already loads (`adminProducts`) filtered to `!p.vendor_id`, rather than adding a new listing endpoint. A small OAuth-return handler switches to the Products tab and opens the panel automatically when the channel's redirect lands back on `/admin.html?channel_oauth=...#products`.

Verified: `node --check` clean on every touched/new file; the full route tree (`server/routes/admin.js` and the whole `server/app.js`) loads without error, confirming `channelAdminController.js` actually resolves (a controller file that was written but not yet copied into place was caught exactly this way before verification, and fixed); full test suite 204/204; no duplicate ids or unbalanced tags in `admin.html`. Playwright-tested end to end against mocked APIs: the panel toggles open, both Applications render with the right type/status/active pill, creating a new Application opens its setup view automatically, the Self Authorization and Web Application setup views show the right fields (including a populated redirect URI), Export to the channel's product list correctly includes Lizimas's own product and excludes a vendor's product, Push Selected calls the bulk-push endpoint with the right id, Import from Channel lists remote items and marks already-linked ones, and the OAuth-return handler cleans the URL after showing its result.

Not yet done: the migration hasn't been run (Ryan's step), and - same caveat as the vendor Channel feature - the Web Application OAuth flow has not been click-tested against a real Channel account.

## Small caption under description-block photos, in both vendor and staff product upload (September 2026)

Why: Ryan wanted a small line of text under a photo in the product description (like the small fine-print under images on other stores' product pages), and specifically asked for it on both vendor's and staff's product upload forms.

Turned out both forms already share one editor component (`client/js/description-block-editor.js`), so this was a single change rather than two. Investigation found the standalone "+ Image" block only ever had an "Alt text" field - which never displays on the page, it exists purely for accessibility/SEO - so there was genuinely no way to put visible words under a plain photo before this. (The separate "+ Grid" block type already had a visible per-image "Caption" field, just styled bold/dark like a small title rather than small fine print - left as-is since it's an established, different-looking feature already in use.)

What shipped: the standalone Image block gained a second input, "Caption shown under the photo (optional)", reusing the block's existing (and previously unused for images) `body` column rather than a new migration. `descriptionBlockController.js` sanitizes it the same way a heading is sanitized (plain text only, 300-character limit, mirroring the existing alt-text limit message). The storefront renderer (`client/js/description-blocks.js`) now prints it under the photo in a new small/gray style (`.pdb-caption` in `product-detail.css`) whenever it's set, and simply omits it when it isn't - existing products with no caption are completely unaffected.

Verified: `node --check` clean on every touched file; full test suite 204/204; full `server/app.js` load test. Playwright-tested end to end on both surfaces: uploading a photo on staff's Add Product form and on vendor's Add Product form each show the new caption input (alt text unaffected), typing into it updates the in-memory block, and saving sends it through to the backend in the `body` field exactly as the controller expects; a separate render-only test confirmed the storefront actually prints a `.pdb-caption` div under a photo that has one and prints nothing extra under one that doesn't.

Not yet done: nothing - this is a small, self-contained, additive change with no new migration and no pending step for Ryan.

## Direct paste into the spec boxes themselves, on staff, vendor desktop, and vendor mobile (September 2026)

Why: Ryan clarified an earlier ask - the existing "Paste from Excel" staging box (a separate textarea + a "Parse & Add Rows" button) wasn't what he meant. He wants to be able to paste directly into the Label/Value boxes themselves, with typing and pasting both working depending on how many specs are being added at once - type one in by hand for a quick edit, or paste a whole block straight onto the boxes for bulk entry, without a separate staging step.

Investigation into a real paste he'd tried (a screenshot of "Os" / "Android 15" / "Ram" / "4 GB" as four separate lines, no tab or colon between label and value) showed why the existing per-line parser wouldn't have helped even if he had used the staging box: it expects each LINE to already contain both the label and the value (via a tab, a colon, or 2+ spaces). His actual clipboard content - likely copied from a rendered spec table where each cell lands on its own line - has no such per-line separator at all; label and value alternate one per line instead.

What shipped: a `paste` event handler on the Label/Value inputs (attached once, via delegation, on the always-present `#specs-list`/`#vm-specs-list` container, so it covers every row past and future) in all three places that have this form - `admin.js` (staff), `vendor-dashboard.js` (vendor desktop), and `vendor-mobile.js` (vendor's mobile Add Product screen). A single-value paste (no tab, no newline) is left completely alone - the browser's normal paste happens and typing is untouched either way. A multi-line/multi-cell paste is intercepted and distributed starting at whichever box was actually pasted into, adding new rows as needed:
- every line has a tab (a real two-column Excel copy) -> one full label+value pair per line;
- every line independently matches the existing colon/2-space pattern -> one pair per line, same rule the staging box already used;
- otherwise (Ryan's actual case - a flat list with no reliable per-line separator) -> consecutive lines are paired alternately as label, value, label, value, ... starting at whichever column received the paste, which is exactly what turns his four-line clipboard content into two correct label/value rows.

The existing "Paste from Excel" staging textarea and its Parse & Add Rows button are untouched and still work exactly as before - this is an additional way in, not a replacement.

Verified: `node --check` clean on all three touched files; full test suite 204/204 (server-side untouched, this is a pure client-side addition). Playwright-tested on all three surfaces using synthetic paste events (a DataTransfer + ClipboardEvent dispatched at a specific box, same as a real paste delivers): confirmed a single value never gets intercepted (`defaultPrevented` stays false, so normal typing and single-value pasting keep working), a tab-separated multi-row paste distributes correctly, a colon-separated multi-row paste distributes correctly, Ryan's exact flat four-line example produces the correct two label/value rows regardless of whether it's pasted starting in the Label or the Value column, pasting a block bigger than the current row count grows the list to fit, and pasting mid-list (not at row 0) fills from that row without disturbing rows above it.

## Vendor photo uploads: reorder and pick the main photo, matching staff (September 2026)

Why: on the vendor product form, photo upload was a bare multi-file `<input type="file">` - vendors could pick several photos but had no way to see them, remove one, or choose which photo ends up first (the "main" photo shoppers see first). Staff/admin's product form already had exactly this - drag-drop, live previews, a numbered list with remove/reorder, and (once a product has 2+ saved photos) a "Save order" button. Ryan asked for vendor to match staff.

What shipped, in `client/vendor/dashboard.html` and `client/js/vendor-dashboard.js`: the bare file input was replaced with a drag-and-drop dropzone + hidden file input + a live preview grid, same pattern as staff. A `vdAllImages` list (of `{key, url}`, where `key` is `id:<dbId>` for a photo already saved on the server or `new:<index>` for one just picked and not yet uploaded) drives a numbered "Photos" order list: each row has a remove (x) button, up/down reorder arrows, and a "NEW" badge for unsaved photos. Stored and new photos can both be reordered in this list before saving.

- Adding a new product: photos are queued in `vdPickedFiles`, and on submit they're appended to the upload in the order shown in the list (not the raw order the file picker returned) - so whichever photo the vendor drags to the top actually becomes the main photo once the product is created.
- Editing an existing product: `editVendorProduct` now also calls `loadVendorProductImagesIntoForm`, which loads the product's already-saved photos (via the existing public `GET /api/products/:id/images` route - no vendor-specific endpoint was needed) into the same order list. Reordering stored photos and clicking "Save order" PATCHes `/api/vendors/products/:id/images/order` (shown once 2+ stored photos are present); removing a single stored photo calls `DELETE /api/vendors/products/images/:imageId`. Both routes already existed and were already ownership-checked (`canEditProduct`) for vendor use - ports of the same staff controller functions - so no server-side changes were needed at all, just the frontend.

Scope: this was built for vendor desktop only, matching the desktop screenshots Ryan sent. Vendor mobile's Add Product screen (`vendor-mobile.js`) still has the old bare file input and hasn't been touched - flag if he wants it matched too.

Verified: `node --check` clean on both touched files; full test suite 204/204 (server-side untouched). Playwright-tested end to end: new-product dropzone/preview/hidden-input swap confirmed; picking 2 new photos shows both in the order list marked NEW and reordering them works; editing an existing product with 3 saved photos loads all 3 into the order list and shows the Save-order button; reordering + Save order sends the PATCH with all 3 ids in the new order and shows a saved confirmation; removing a stored photo sends the DELETE and drops it from the list.

## Vendor registration: fixed the "account already exists" false block, plus an alert for abandoned signups (September 2026)

Bug fixed: the vendor registration wizard's email-exists check (both at the "send me a code" step and at final submit) looked up `users` by email with no role filter, so ANY existing account with that email - a customer, staff, whatever - showed "An account with this email already exists. Please log in instead." even though that email had never registered as a vendor. Both checks in `server/controllers/authController.js` (`requestVendorRegistrationCode` and `registerVendor`) now join through to the `vendors` table and only block when this email already has an actual vendor account. A customer or staff email can now go through vendor registration and get its own, separate vendor login, same as a brand-new email would. Also hardened the final `INSERT INTO users` against a genuine unique-constraint violation on email (the base `users` table predates this repo's migrations, so its exact constraints aren't visible here) - if the DB itself refuses a duplicate email, that now surfaces as a clear message instead of a bare 500.

New: an email alert to Ryan (`ADMIN_ALERT_EMAIL`, the same address the existing admin-login/block alerts already use) when someone starts a vendor registration and never finishes it. `vendor_registration_otp` (migration 080) already holds one row per email from the moment they request a verification code until they either finish registering (row deleted) or the row goes stale - so a new sweep job, `server/jobs/vendorSignupAbandonment.js` (same setInterval-on-the-web-dyno pattern as the payment reconciler, since there's no separate worker on Render), checks every 10 minutes for rows untouched for 30+ minutes with no alert sent yet, and emails Ryan the address, when they started, and whether they got as far as entering the verification code before stopping. The same sweep also emails the applicant themselves a gentle reminder - it's often a dropped connection rather than a change of mind, so it says as much and links back to the registration page, noting they'll need to re-verify their email since the original code will have expired by then. Migration 086 adds the `alerted_at` tracking column (reset on a fresh code request, so a second abandoned attempt later still gets its own alert). Off automatically if `ADMIN_ALERT_EMAIL` isn't set; otherwise on by default (`VENDOR_SIGNUP_ABANDONMENT_ALERTS_ENABLED=false` to disable explicitly), mirroring how the payment reconciler is toggled.

Verified: `node --check` clean on all touched/new files (authController.js, mailer.js, server.js, the new job file); full test suite still 204/204 (this area had no existing tests - all logic-only unit tests, none touching these DB-backed controllers); `server/app.js`, `server/routes/admin.js`, the new job module, and `authController.js` all `require()` cleanly with fake env vars, confirming no load-time errors. Not verified against a live database (no network access to Postgres from here) - Ryan should run migration 086 the usual way and can watch the server logs for "Vendor signup abandonment alerts started" after deploying.

## Premium vendor desktop dashboard: end-to-end verification pass (September 2026)

The redesign above (and its two follow-up passes) had shipped across several earlier sessions but every one of those entries explicitly flagged the same gap: "not covered by automated tests - needs a live browser pass." Ryan asked for everything on the vendor center to be finished and verified end to end, so this pass closed that gap with a full Playwright run against the actual shipped `client/vendor/dashboard.html` / `vendor-dashboard.js` / `vendor-desktop.css`, no code changes - purely verification.

Confirmed working: the Overview tab's stat cards, Store Rating (computed correctly from mocked review rows - 5/4/4 averaged to 4.3 with the right star count), Recent Orders, Top Performing Products, and Notifications preview all populate from the existing endpoints; the sidebar accordion groups (Orders/Products/Analytics) start collapsed, open and close on click, and auto-open when one of their own tabs is active; the bottom profile mini-card's Settings/Profile/Logout menu toggles correctly; the topbar product search switches to the Products tab, auto-expands its nav group, and correctly filters to matching products only; the Inventory tab's four stat counts (Total/In Stock/Low Stock/Out of Stock) matched hand-computed expected values against a 4-product mock, and its Low-and-Out vs All Products filter toggle showed the right subset each time; the bulk-action flow (select products -> confirmation modal naming the count and action -> PATCH request with the right ids -> results modal breaking down successful/failed/skipped with the failure reason shown -> download-report button appearing only when there were failures/skips) worked end to end against a mocked bulk endpoint; and the vendor mini-profile card/avatar/welcome heading/storefront link all populate correctly from `/api/vendors/me`.

One thing couldn't be verified here: Chart.js itself is loaded from a CDN (cdnjs.cloudflare.com), which this environment's network policy blocks - so the Sales Overview chart's canvas renders but the chart library never loads in this sandbox. The code already guards for that (`if (!canvas || typeof Chart === "undefined") return;`), so nothing throws or breaks; this is an environment limitation, not a product gap, but the actual chart rendering (and that the Overview and Reports tabs' independent Chart.js instances truly don't conflict on a real page load) is worth a quick look in a real browser once deployed, where the CDN isn't blocked.

Verified: full test suite 204/204 (no code touched by this pass - it was existing shipped code); a dedicated Playwright harness (`/tmp/test_vendor_premium_dashboard.py` in the session's cloud workspace) exercising every item above, all assertions passing; no console/page errors other than the expected CDN block noted above.

## Fixed: Sales Overview chart rendering huge and blank on the live vendor dashboard (September 2026)

Ryan sent a screenshot from the live site: the Overview tab's Sales Overview chart showed an axis stuck at "1.0" over a huge blank white area instead of an actual chart. Root cause: both chart canvases (`vd-sales-overview-chart` on the Overview tab and `vendor-reports-chart` on the Reports tab) are initialized with Chart.js's `maintainAspectRatio: false`, which requires the canvas's immediate parent to have an explicit, bounded height - neither `.vd-card` nor `.panel` (their respective parents) define one, so the canvas had nothing determinate to size itself against. This is a well-known Chart.js pitfall, and it had been flagged as unverified in the previous end-to-end pass specifically because Chart.js itself is loaded from a CDN (cdnjs.cloudflare.com) that this environment's network policy blocks - so the chart's actual visual rendering could only ever be confirmed live, which is exactly what just happened.

Fix: both canvases are now wrapped in a new `.vd-chart-container` div (`position: relative; height: 260px`, canvas filling it at 100%/100%), scoped under `.vendor-desktop-shell` so nothing outside the vendor dashboard is touched. Confined to `client/vendor/dashboard.html` and `client/css/vendor-desktop.css`; no JS changes needed - Chart.js reads the parent's size on its own.

Verified: `node --check` n/a (no JS touched); HTML tag balance and duplicate-id checks clean; full test suite 204/204; a Playwright test using a lightweight stand-in for Chart.js (records the constructor's measured parent size rather than actually drawing) confirmed both containers now report a bounded 260px height at chart-construction time instead of the previous unbounded/collapsed size, and the full existing dashboard end-to-end suite still passes with no new console errors. The real Chart.js library's actual pixel-perfect rendering still can't be watched from this sandbox (same CDN block as before) - worth Ryan's own glance at the live site after this deploys to confirm the chart itself now draws normally, though the sizing root cause is fixed either way.

## Shop ID, Sponsored-products storefront injection, and consignment order-routing (September 2026)

Why: Ryan asked for a matching Shop ID on Lizimas vendors, for the "Advertise Your Products" ad campaigns built earlier to actually show sponsored goods on the storefront (their own migration/controller comments explicitly flagged this as a deliberate not-built follow-up), and for any other still-open Channel-comparison gap to be closed - which meant finishing the other explicitly-flagged follow-up too: routing a placed order to fulfill straight from a vendor's `consigned_stock` instead of always going through per-order vendor handover.

**Shop ID** (migration 113): `vendors.shop_id` + a `vendor_shop_id_seq` sequence, generated by `server/utils/shopId.js` in the channel's own visual shape (`UG` + a sequential number + 2 random uppercase letters - the sequence alone guarantees uniqueness, the letters just match the look). Assigned once, automatically, the moment a vendor is approved (`vendorController.js`'s `approveVendor`); the migration also backfills a shop_id for every vendor already approved before it runs. Shown read-only on the vendor's own dashboard (a new "Shop ID" panel on desktop's Account tab, a new row in mobile's Profile screen) and as a column in the admin panel's Vendor Compliance table, with an "Assign" button (`PATCH /api/admin/vendors/:id/regenerate-shop-id`) as a rare escape hatch for a vendor that somehow still has none.

**Sponsored-products storefront injection**: closes the gap `migrations/112_vendor_ad_campaigns.sql` and `adTrackingController.js` both explicitly left open. Two changes, both reusing plumbing that already existed rather than inventing new mechanisms: (1) `GET /api/ads/sponsored-products` (already built, already mounted at `/api/ads`, just never called from anywhere) now returns full card-shaped data (image, stock, vendor-availability filtering matching the public catalogue) and powers a new dedicated "Sponsored" carousel row on the homepage (`#ls-sponsored` in `client/index.html`, populated by `loadSponsoredRow()` in `products.js`, reusing `buildProductCard()` - same card markup as everywhere else on the site). (2) `productController.js`'s main `getProducts` query and `vendorController.js`'s `getPublicStorefront` (a vendor's own `/store/:slug` page) now fold active, unexhausted `vendor_ad_campaign_products` into the existing `is_sponsored` ranking/badge that `vendor_promotions.sponsored` already drove - so an ad-campaign product also gets the "Sponsored" badge and sorts to the top of search/category/storefront listings, not just the dedicated homepage row. A new `ad_sponsored` boolean distinguishes a CPC-billed ad placement from the older non-billed `vendor_promotions` sponsorship, so `buildProductCard()`/`stBuildProductCard()` only fire the click-billing beacon (`POST /api/ads/track-click`, via `fetch(..., {keepalive: true})` so it survives the same-click navigation) for an actual ad campaign. Impressions are logged (fire-and-forget) everywhere an ad-sponsored product is returned in a listing, not just the dedicated endpoint.

**Consignment order-routing integration** (migration 114): closes the other gap `migrations/110_vendor_consignments.sql` explicitly left open. `checkoutController.js` now checks, per plain (non-variant) line item, whether the product is `fulfillment_type = 'lizimas_fulfilled'` with `consigned_stock` covering the *entire* line item quantity (extracted as a small pure/testable decision, `server/utils/consignmentFulfillment.js`'s `shouldFulfillFromConsignment`) - if so, the item is inserted already `handover_status = 'accepted'` (Lizimas already physically holds it - it was counted in when the consignment was received, so there's nothing left for the vendor to hand over or an admin to inspect) instead of the normal `pending_handover` -> `vendor_fulfilment_stage = 'new'` start, and `products.consigned_stock` is decremented alongside the existing `products.stock` decrement. A new `order_items.fulfilled_from_consignment` boolean is a pure audit/reporting marker - nothing else needed to change, since `handover_status IS NOT NULL` remains the existing vendor-attribution signal everywhere else already relies on. Deliberately no split-fulfillment: if `consigned_stock` can't cover the whole line item, it falls back to the normal vendor-handover flow unchanged. Variant items are unaffected (`product_variants` has no `fulfillment_type`/`consigned_stock` of its own).

Verified: `node --check` clean on every touched/new file; full test suite 284/284 (10 new unit tests - 4 for `generateShopId`'s format/padding, 6 for `shouldFulfillFromConsignment`'s routing decision across vendor/fulfillment-type/stock-coverage combinations); `server/app.js` load-checked cleanly with dummy env vars. Playwright-tested end to end against mocked APIs: the homepage Sponsored row renders, shows the Sponsored badge, hides itself when there are no active campaigns, and clicking a card fires the click-billing beacon with the right product id before navigating; a vendor storefront page shows the Sponsored badge only on the ad-campaign product (not a plain one on the same page) and fires the same beacon; the vendor dashboard's Shop ID panel (desktop) and Profile row (mobile) both show the assigned Shop ID; the admin Vendor Compliance table shows an existing Shop ID and its Assign button calls the regenerate endpoint and refreshes correctly.

Not yet done: migrations 113/114 haven't been run (Ryan's step, as always) - migration 113's backfill assigns a shop_id to every already-approved vendor the moment it runs. The consignment order-routing path can't be exercised against a real database from this sandbox (no network access to Postgres) - worth Ryan creating one consignment, having it received, and placing a real test order against that product once the migrations are live, to watch it come through pre-accepted with no vendor-handover step. `.claude_test_bundle.tar.gz`, a throwaway tarball used to pull the repo into the cloud sandbox for Playwright testing this pass, was moved to `_to_delete/` in the repo root rather than deleted outright (no delete permission requested) - safe for Ryan to delete that folder.
