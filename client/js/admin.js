...
const ADMIN_SPECS_GRID_ROWS = 8;
const ADMIN_SPECS_GRID_COLS = 10;

function toggleAdminSpecsGridPicker() {
    ...
    if (grid && !grid.children.length) {
        let html = "";
        for (let r = 0; r < ADMIN_SPECS_GRID_ROWS; r++) {
            for (let c = 0; c < ADMIN_SPECS_GRID_COLS; c++) {
                html += `<span class="specs-grid-cell" data-row="${r}" data-col="${c}"></span>`;
            }
        }
        grid.innerHTML = html;
        grid.addEventListener("mouseover", adminSpecsGridHover);
        grid.addEventListener("click", adminSpecsGridPick);
    }
    popover.classList.remove("hidden");
}

function adminSpecsGridHover(e) {
    ...
    const label = document.getElementById("admin-specs-grid-label");
    if (label) label.textContent = `${col + 1} x ${row + 1}`;
}

function adminSpecsGridPick(e) {
    const cell = e.target.closest(".specs-grid-cell");
    if (!cell) return;
    const rows = parseInt(cell.dataset.row, 10) + 1;
    const cols = parseInt(cell.dataset.col, 10) + 1;
    document.getElementById("admin-specs-grid-popover").classList.add("hidden");
...
function setupAdminSpecsPasteHandler() {
    ...
    list.addEventListener("paste", (e) => {
        ...
        adminDistributeSpecPaste(rowIndex, col, text);
    });
}

document.addEventListener("DOMContentLoaded", setupAdminSpecsPasteHandler);
...
async function ajLoadChannelExport() {
    ...
    try {
        ...
        // Lizimas's own products only (vendor_id IS NULL), matching what
        // the backend will actually accept a push for.
    ...
    container.innerHTML = `
        <table>
            ...
            <tbody>
                ${products.map(p => `
                    <tr>
                        <td data-label="Product"><a href="javascript:void(0)" onclick="viewPendingProduct(${p.id})" style="color:#1D4ED8; text-decoration:underline; cursor:pointer;">${escapeReportText(p.name)}</a></td>
...
// Opens the product exactly as it was submitted, read-only (admin-product-view.js).
function viewPendingProduct(id) {
    openAdminProductView(id);
}
...
// (kept identical to client/js/staff-messages-widget.js).
const TM_EMOJI_CATEGORIES = [
...
function tmRenderEmojiGrid() {
    const grid = document.getElementById("tm-emoji-grid");
    if (!grid) return;
    const emojis = TM_EMOJI_CATEGORIES[tmActiveEmojiCategory].emojis;
    grid.innerHTML = emojis.map(e => `<button type="button" class="tm-emoji-item">${e}</button>`).join("");
    grid.querySelectorAll(".tm-emoji-item").forEach(btn => {
...
function initTeamMessagesComposer() {
    ...
    if (emojiBox) {
        emojiBox.classList.add("tm-emoji-popover-wrap");
        emojiBox.innerHTML = `
            <div class="tm-emoji-tabs" id="tm-emoji-tabs">
                ${TM_EMOJI_CATEGORIES.map((c, i) => `<button type="button" class="tm-emoji-tab${i === 0 ? " active" : ""}" data-cat="${i}">${c.icon}</button>`).join("")}
            </div>
            <div class="tm-emoji-grid" id="tm-emoji-grid"></div>`;
        emojiBox.querySelectorAll(".tm-emoji-tab").forEach(tab => {
            tab.addEventListener("click", () => {
                tmActiveEmojiCategory = Number(tab.dataset.cat);
                emojiBox.querySelectorAll(".tm-emoji-tab").forEach(t => t.classList.toggle("active", t === tab));
                tmRenderEmojiGrid();
            });
        });
        tmRenderEmojiGrid();
