...
// spreadsheet-style, adding new rows as needed). Mirrors
// adminDistributeSpecPaste/vendorDistributeSpecPaste - kept as its own
// copy, same reason as the rest of this parser (separate files). ---

function vmDistributeSpecPaste(startRowIndex, startCol, text) {
    const rawLines = text.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    if (rawLines.length === 0) return;

    function ensureRow(idx) {
        let rows = document.querySelectorAll("#vm-specs-list > div");
        while (rows.length <= idx) {
            vmAddSpecRow();
            rows = document.querySelectorAll("#vm-specs-list > div");
        }
        return rows[idx];
    }
    ...
    function setCell(idx, col, val) {
        ...
        const input = row.querySelector(col === "label" ? ".vm-spec-label-input" : ".vm-spec-value-input");
...
function setupVmSpecsPasteHandler() {
    const list = document.getElementById("vm-specs-list");
    ...
    list.addEventListener("paste", (e) => {
        ...
        let col = null;
        if (target.classList.contains("vm-spec-label-input")) col = "label";
        else if (target.classList.contains("vm-spec-value-input")) col = "value";
        if (!col) return;
        ...
        e.preventDefault();
        const rows = Array.from(document.querySelectorAll("#vm-specs-list > div"));
        const row = target.closest("#vm-specs-list > div");
        const rowIndex = Math.max(0, rows.indexOf(row));
        vmDistributeSpecPaste(rowIndex, col, text);
    });
}
