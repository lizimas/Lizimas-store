// Marketplace channel API client (Ryan, Sept 2026): pushes a Lizimas listing
// out to a vendor's store on a connected sales channel and pulls existing
// channel listings into Lizimas.
//
// Every address comes from the environment - nothing is hard-coded:
//   CHANNEL_API_BASE        API host (token + catalogue endpoints)
//   CHANNEL_AUTHORIZE_BASE  browser consent host for Web Application sign-in
//   CHANNEL_AUTHORIZE_PATH  consent path (default /oauth/authorize)
// Without CHANNEL_API_BASE the integration reports "not configured".

const axios = require("axios");
const { encryptField, decryptField } = require("../utils/encryption");

const CHANNEL_API_BASE = process.env.CHANNEL_API_BASE || "";
const CHANNEL_TOKEN_PATH = "/token";
const CHANNEL_PRODUCTS_PATH = "/catalog/products";
const CHANNEL_CATEGORY_TREE_PATH = "/categories";
const CHANNEL_AUTHORIZE_BASE = process.env.CHANNEL_AUTHORIZE_BASE || "";
const CHANNEL_AUTHORIZE_PATH = process.env.CHANNEL_AUTHORIZE_PATH || "/oauth/authorize";
...
function requireConfigured() {
    if (!CHANNEL_API_BASE) {
        throw new ChannelApiError("The sales channel connection isn't set up on the server yet (CHANNEL_API_BASE is missing).", { code: "not_configured" });
    }
}

function channelHttp() {
    requireConfigured();
    return axios.create({
        baseURL: CHANNEL_API_BASE,
        timeout: REQUEST_TIMEOUT_MS,
        headers: { "Content-Type": "application/json", Accept: "application/json" }
    });
}

// Mints a short-lived access token from a vendor's Channel Application
// Client ID + Refresh Token, per the "Case 2 (Refresh Token)" branch of
// the channel's own /token endpoint: form-urlencoded body (NOT JSON - the
...
// credential, used as-is on every call. Uncertain whether Channel rotates
...
// either way unless Channel actively invalidates the old one.
async function mintAccessToken(clientId, refreshToken, clientSecret) {
    ...
    try {
        ...
        // Channel specifically, but harmless to include when present.
        if (clientSecret) fields.client_secret = clientSecret;
        const body = new URLSearchParams(fields);
        const response = await axios.post(`${CHANNEL_API_BASE}${CHANNEL_TOKEN_PATH}`, body.toString(), {
            timeout: REQUEST_TIMEOUT_MS,
            headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" }
        });
        const data = response.data || {};
        if (!data.access_token) {
            throw new ChannelApiError("Channel did not return an access token.", { raw: data });
        }
        return {
        ...
        throw normalizeAxiosError(err, "Could not connect to Channel with the Client ID/Refresh Token provided.");
    }
}
...
// from channelSyncService) the caller uses to identify which vendor/
// Application the callback belongs to - Channel is expected to hand it
...
// best-effort guess, not confirmed against the channel's real authorize screen.
function buildAuthorizeUrl(clientId, redirectUri, state) {
    if (!CHANNEL_AUTHORIZE_BASE) {
        throw new ChannelApiError("Web Application sign-in isn't set up on the server yet (CHANNEL_AUTHORIZE_BASE is missing).", { code: "not_configured" });
    }
    const url = new URL(CHANNEL_AUTHORIZE_PATH, CHANNEL_AUTHORIZE_BASE);
...
async function exchangeAuthorizationCode(clientId, clientSecret, redirectUri, code) {
    ...
    try {
        ...
        throw normalizeAxiosError(err, "Could not complete sign-in with Channel.");
    }
}
...
// note in channelSyncService.js for why the column keeps its original
...
// itself - the caller (channelSyncService) owns writing the connection row
...
async function ensureFreshAccessToken(connectionRow) {
    ...
    if (connectionRow.app_type === "web_application") {
        ...
        if (!refreshToken) {
            throw new ChannelApiError("This Application has no refresh token yet - sign in with Channel again from the Applications tab.", { code: "NO_REFRESH_TOKEN" });
        }
    ...
    if (!refreshToken) {
        throw new ChannelApiError("This Channel connection is missing its Refresh Token - reconnect with the Client ID/Refresh Token.", { code: "NO_REFRESH_TOKEN" });
    }
...
function authedHttp(accessToken) {
    const client = channelHttp();
    client.defaults.headers.common.Authorization = `Bearer ${accessToken}`;
    return client;
}

// --- Field mapping: Lizimas product -> Channel product payload ---
...
// (label/value pairs) - passed through as free-form Channel "attributes"
// until real per-category attribute names are confirmed.
function mapLizimasProductToChannelPayload(product, specs, images) {
    return {
...
// --- Field mapping: Channel product -> Lizimas product fields ---
// Used when importing a vendor's existing Channel listing into Lizimas.
...
function mapChannelProductToLizimasFields(channelProduct) {
    const attributes = channelProduct.attributes || {};
    return {
        productFields: {
            name: channelProduct.name || "",
            description: channelProduct.description || "",
            brand: channelProduct.brand || null,
            price: channelProduct.price != null ? Number(channelProduct.price) : 0,
            stock: channelProduct.quantity != null ? Number(channelProduct.quantity) : 0,
            sku: channelProduct.seller_sku || null
        },
        specs: Object.entries(attributes).map(([label, value], index) => ({
            label, value: value == null ? "" : String(value), display_order: index + 1
        })),
        images: Array.isArray(channelProduct.images) ? channelProduct.images : []
    };
}

// Creates or updates one product on Channel. channelProductId present ->
// update (PATCH); absent -> create (POST). Per the same third-party
// integration referenced above, the channel's catalog API is feed-based: both
...
// this module does not yet implement). Returns the channel's product id for a
// create, so the caller can store it on the link row.
async function upsertChannelProduct(accessToken, payload, channelProductId) {
    try {
        const http = authedHttp(accessToken);
        const body = channelProductId ? { ...payload, channel_product_id: channelProductId } : payload;
        const response = channelProductId
            ? await http.patch(CHANNEL_PRODUCTS_PATH, body)
            : await http.post(CHANNEL_PRODUCTS_PATH, body);
        const data = response.data || {};
        const newId = channelProductId || data.product_id || data.id || null;
        return { channelProductId: newId, raw: data };
    } catch (err) {
        throw normalizeAxiosError(err, `Channel rejected the product "${payload.name || payload.seller_sku}".`);
    }
}

// Lists the vendor's existing products on Channel (for the "import from
// Channel" screen). Supports simple pagination since a vendor could have
// hundreds of listings.
async function listChannelProducts(accessToken, { page = 1, pageSize = 50 } = {}) {
    try {
        const http = authedHttp(accessToken);
        const response = await http.get(CHANNEL_PRODUCTS_PATH, { params: { page, page_size: pageSize } });
        const data = response.data || {};
        return {
        ...
        throw normalizeAxiosError(err, "Could not fetch your Channel product list.");
    }
}

function normalizeAxiosError(err, fallbackMessage) {
    if (err instanceof ChannelApiError) return err;
    const status = err.response ? err.response.status : null;
    const raw = err.response ? err.response.data : null;
    const channelMessage = raw && (raw.error_description || raw.message || raw.error);

    // The endpoint paths/response shape in this file are unverified against
    // a real Channel Application (see the header comment) - so the FIRST live
    ...
    let message = channelMessage || fallbackMessage;
    if (!channelMessage) {
        if (status) {
            const bodySnippet = raw ? (typeof raw === "string" ? raw : JSON.stringify(raw)).slice(0, 300) : "";
            message += ` (Channel responded with HTTP ${status}${bodySnippet ? ": " + bodySnippet : ""})`;
        } else if (err.code) {
    ...
    console.error("[channelClient] request failed:", { status, code: err.code || null, raw, originalMessage: err.message });
    return new ChannelApiError(message, { status, raw });
}
...
module.exports = {
    ...
    mapLizimasProductToChannelPayload,
    mapChannelProductToLizimasFields,
