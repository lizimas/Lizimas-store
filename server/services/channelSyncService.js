// Vendor marketplace channel sync (Ryan, Sept 2026): a vendor's connected
// channel Applications, product links, import and export. Credentials are
// stored encrypted (AES-256-GCM, same helper as vendor KYC).
...
const CHANNEL_OAUTH_CALLBACK_URL = `${PUBLIC_BASE_URL}/api/vendors/channel/oauth/callback`;

async function logSync(vendorId, linkId, action, status, detail) {
    await pool.query(
        `INSERT INTO channel_sync_log (vendor_id, product_link_id, action, status, detail)
         VALUES ($1, $2, $3, $4, $5)`,
        [vendorId, linkId || null, action, status, detail || null]
    );
}

// --- Applications (Task: multiple named Channel Applications per vendor,
// Manage Applications screen - see migration 084) ---
...
async function listApplications(vendorId) {
    const rows = await pool.query(
        `SELECT id, name, app_type, client_id, redirect_uri, connection_status,
         ...
         FROM vendor_channel_connections WHERE vendor_id = $1 ORDER BY created_at ASC`,
        [vendorId]
    );
...
async function getConnectionStatus(vendorId) {
    const row = await pool.query(
        `SELECT connection_status, channel_shop_name, last_connected_at, last_error, client_id
         FROM vendor_channel_connections WHERE vendor_id = $1 AND is_active = true`,
        [vendorId]
    );
...
async function getApplicationOwned(vendorId, applicationId) {
    const row = await pool.query(
        `SELECT * FROM vendor_channel_connections WHERE id = $1 AND vendor_id = $2`,
...
async function createApplication(vendorId, { name, appType, redirectUri }) {
    ...
    // Same name twice in one shop is refused (Vendor Center parity:
    ...
    const duplicate = await pool.query(
        `SELECT 1 FROM vendor_channel_connections
    ...
    const existingCount = (await pool.query(
        `SELECT count(*)::int AS n FROM vendor_channel_connections WHERE vendor_id = $1`, [vendorId]
    )).rows[0].n;
    const inserted = await pool.query(
        `INSERT INTO vendor_channel_connections (vendor_id, name, app_type, client_id, client_secret_enc, connection_status, redirect_uri, is_active)
...
async function deleteApplication(vendorId, applicationId) {
    ...
    await pool.query(`DELETE FROM vendor_channel_connections WHERE id = $1`, [applicationId]);
...
async function setActiveApplication(vendorId, applicationId) {
    ...
    try {
        ...
        await client.query(`UPDATE vendor_channel_connections SET is_active = false WHERE vendor_id = $1`, [vendorId]);
        await client.query(`UPDATE vendor_channel_connections SET is_active = true, updated_at = now() WHERE id = $1`, [applicationId]);
...
// clientSecret param here is really the vendor's Channel Refresh Token for a
// Self Authorization Application (see channelClient.js's header note).
async function connectApplication(vendorId, applicationId, clientId, refreshToken) {
    ...
    try {
        ...
        await pool.query(
            `UPDATE vendor_channel_connections
...
async function activateIfNoneActive(vendorId, applicationId) {
    const hasActive = (await pool.query(
        `SELECT 1 FROM vendor_channel_connections WHERE vendor_id = $1 AND is_active = true`, [vendorId]
    )).rows.length > 0;
    if (!hasActive) {
        await pool.query(`UPDATE vendor_channel_connections SET is_active = true, updated_at = now() WHERE id = $1`, [applicationId]);
    }
}

// --- Web Application OAuth (Authorization Code Flow) ---
// UNVERIFIED end to end - see channelClient.js's header for exactly what
// part of this is and isn't confirmed against the channel's real API.

// Sets/updates a Web Application's Client ID + Client Secret, collected
// from the vendor just before "Sign in with Channel" (the channel's own dialog
...
// the matching Application on Channel and copied its credentials here).
async function setWebApplicationCredentials(vendorId, applicationId, clientId, clientSecret) {
    ...
    await pool.query(
        `UPDATE vendor_channel_connections SET client_id = $1, client_secret_enc = $2, updated_at = now() WHERE id = $3`,
...
// Handles the channel's redirect back after the vendor consents. Called from a
...
async function handleOAuthCallback(code, state) {
    ...
    try {
        ...
        await pool.query(
            `UPDATE vendor_channel_connections SET connection_status = 'error', last_error = $1, updated_at = now() WHERE id = $2`,
...
async function getFreshConnection(vendorId) {
    const row = await pool.query(`SELECT * FROM vendor_channel_connections WHERE vendor_id = $1 AND is_active = true`, [vendorId]);
...
async function findOrCreateLink(vendorId, productId, sellerSku) {
    const existing = await pool.query(
        `SELECT * FROM channel_product_links WHERE vendor_id = $1 AND product_id = $2`,
        [vendorId, productId]
    );
    if (existing.rows.length > 0) return existing.rows[0];
    const created = await pool.query(
        `INSERT INTO channel_product_links (vendor_id, product_id, channel_seller_sku, sync_direction, sync_status)
         VALUES ($1, $2, $3, 'push', 'pending')
         ON CONFLICT (vendor_id, channel_seller_sku) DO UPDATE SET product_id = EXCLUDED.product_id
         RETURNING *`,
...
// never throws for an ordinary Channel-side rejection, so bulk push can
...
async function pushProduct(vendorId, productId) {
    ...
    try {
        ...
        await logSync(vendorId, link.id, "push", "success", `Pushed "${product.name}" to Channel.`);
        return { status: "success" };
    } catch (err) {
        await pool.query(
            `UPDATE channel_product_links SET sync_status = 'failed', last_error = $1, updated_at = now() WHERE id = $2`,
            [err.message, link.id]
        );
...
async function listRemoteProducts(vendorId, page) {
    ...
    const linked = await pool.query(
        `SELECT channel_seller_sku FROM channel_product_links WHERE vendor_id = $1`,
        [vendorId]
    );
...
// Imports a set of already-fetched Channel product objects (as returned by
// listRemoteProducts) into Lizimas as new products, each linked back to
// its Channel listing. status is 'created'/'linked'/'skipped' per item to
...
async function importProducts(vendorId, remoteProducts) {
    ...
    for (const remote of remoteProducts) {
        ...
        const existingLink = await pool.query(
            `SELECT * FROM channel_product_links WHERE vendor_id = $1 AND channel_seller_sku = $2`,
            [vendorId, sellerSku]
        );
        ...
        if (existingLink.rows.length > 0) {
            await pool.query(
                `UPDATE channel_product_links SET product_id = $1, sync_status = 'synced', last_synced_at = now(),
            ...
            await logSync(vendorId, existingLink.rows[0].id, "import", "success", `Imported "${newProduct.name}" from Channel.`);
        } else {
            const linkInsert = await pool.query(
                `INSERT INTO channel_product_links
                    (vendor_id, product_id, channel_seller_sku, channel_product_id, sync_direction, sync_status, last_synced_at, last_synced_product_updated_at)
                 VALUES ($1, $2, $3, $4, 'pull', 'synced', now(), $5)
                 RETURNING id`,
                [vendorId, newProduct.id, sellerSku, remote.product_id || remote.id || null, newProduct.updated_at]
            );
            await logSync(vendorId, linkInsert.rows[0].id, "import", "success", `Imported "${newProduct.name}" from Channel.`);
        }
...
// --- Sync status list (for the vendor-facing "Channel" tab) ---
